function validateForm(){
    if(campoName.value.trim()!=""&&campoApellido.value.trim()!=""){
        localStorage.setItem("name",campoName.value.trim());
        localStorage.setItem("apel",campoApellido.value.trim());    
    }else{alert("no")}
    if(validateEmail()&&validatePass()&&validateUser()){
        alert("Bienvenido al sistema "+localStorage.getItem("name")+" "+localStorage.getItem("apel"));
    }
}

var campoName = document.querySelector("#inputName");
var campoApellido = document.querySelector("#inputApel");
