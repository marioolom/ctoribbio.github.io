...
button.addEventListener("click", e => {
    ...
    req.open("GET", "json/catalogo.json", true);
...
function generarTabla(req) {
    ...
    var cdListTemp = docJSON.CATALOG.CD;

    for (var i = 0; i < cdListTemp.length; i++) {
        var titulo = cdListTemp[i].TITLE;
        var artista = cdListTemp[i].ARTIST;
        var country = cdListTemp[i].COUNTRY;
        var company = cdListTemp[i].COMPANY;
        var price = cdListTemp[i].PRICE;
        var year = cdListTemp[i].YEAR;
    ...
    document.querySelector("#lipsum").insertAdjacentElement("afterend", div);
    const columnDefs = [
        { field: "title" },
        { field: "artist" },
        { field: "country" },
